...
2. **BSRQ (基地台主動問)**：這是針對長延遲或大量設備開發的優化機制。基地台不再只是被動等待，而是會主動詢問 (Query) 手機的緩衝區狀態，或是透過預測機制減少一來一回的次數。
3. **隨機存取程序 (RACH)**：當手機連 BSR 的通道都沒有時，需在公共頻道發起請求。
...
* **RFsimulator**：在沒有實體天線的情況下，於軟體層面模擬無線訊號傳遞，適合在 WSL2/Linux 上初步開發。
